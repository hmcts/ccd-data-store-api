...
class UICaseSearchControllerTest {
    ...
    @Mock
    private CaseSearchResultGenerator caseSearchResultGenerator;
    ...
    void shouldSearchCaseDetails() {
        ...
        when(caseSearchResultGenerator.execute(any(), any(), any())).thenReturn(uiCaseSearchResult);
        ...
        verify(elasticsearchQueryHelper).prepareRequest(eq(CASE_TYPE_ID), eq(searchRequest), eq("WORKBASKET"));
        verify(caseSearchOperation).execute(eq(preparedRequest));
        verify(caseSearchResultGenerator).execute(eq(CASE_TYPE_ID), eq(caseSearchResult), eq(WORKBASKET));
        assertAll(
