    ...
    class GrantAccess {
        ...
        class AMToCCD {
            ...
            void shouldInvokeServicesInOrder() {
                caseUserRepository.grantAccess(CCD_CASE_TYPE_ID, CCD_CASE_ID, USER_ID, CREATOR.getRole());
                ...
                assertAll(
                    ...
                    () -> inOrder.verify(ccdCaseUserRepository).grantAccess(CCD_CASE_TYPE_ID, CCD_CASE_ID, USER_ID, CREATOR.getRole()),
        ...
        class AMToAM {
            ...
            @DisplayName("should invoke services in order for granting access to user")
            void shouldInvokeServicesInOrder() {
                caseUserRepository.grantAccess(AM_CASE_TYPE_ID, AM_CASE_ID, USER_ID, CREATOR.getRole());
                ...
                assertAll(
                    ...
                    () -> inOrder.verify(amCaseUserRepository).grantAccess(AM_CASE_TYPE_ID, AM_CASE_ID, USER_ID, CREATOR.getRole()),
        ...
        class AMToBoth {
            ...
            @DisplayName("should invoke services in order for granting access to user")
            void shouldInvokeServicesInOrder() {
                caseUserRepository.grantAccess(BOTH_CASE_TYPE_ID, BOTH_CASE_ID, USER_ID, CREATOR.getRole());

                InOrder inOrder = inOrder(caseDetailsRepository, amSwitch, ccdCaseUserRepository, amCaseUserRepository);
                assertAll(
                    () -> inOrder.verify(amSwitch).isWriteAccessManagementWithCCD(BOTH_CASE_TYPE_ID),
                    () -> inOrder.verify(ccdCaseUserRepository).grantAccess(BOTH_CASE_TYPE_ID, BOTH_CASE_ID, USER_ID, CREATOR.getRole()),
                    () -> inOrder.verify(amSwitch).isWriteAccessManagementWithAM(BOTH_CASE_TYPE_ID),
                    () -> inOrder.verify(amCaseUserRepository).grantAccess(BOTH_CASE_TYPE_ID, BOTH_CASE_ID, USER_ID, CREATOR.getRole()),
    ...
    class RevokeAccess {
        ...
        class AMToCCD {
            ...
            void shouldRevokeAccess() {
                caseUserRepository.revokeAccess(CCD_CASE_TYPE_ID, CCD_CASE_ID, USER_ID, CREATOR.getRole());

                assertAll(
                    () -> verify(amSwitch).isWriteAccessManagementWithCCD(CCD_CASE_TYPE_ID),
                    () -> verify(ccdCaseUserRepository).revokeAccess(CCD_CASE_TYPE_ID, CCD_CASE_ID, USER_ID, CREATOR.getRole()),
                    () -> verifyZeroInteractions(amCaseUserRepository)
                );
            }
            ...
            void shouldInvokeServicesInOrder() {
                ...
                assertAll(
                    () -> inOrder.verify(amSwitch).isWriteAccessManagementWithCCD(CCD_CASE_TYPE_ID),
                    () -> inOrder.verify(ccdCaseUserRepository).revokeAccess(CCD_CASE_TYPE_ID, CCD_CASE_ID, USER_ID, CREATOR.getRole()),
                    () -> inOrder.verify(amSwitch).isWriteAccessManagementWithAM(CCD_CASE_TYPE_ID),
                    inOrder::verifyNoMoreInteractions,
                    () -> verifyNoMoreInteractions(ccdCaseUserRepository),
                    () -> verifyZeroInteractions(amCaseUserRepository)
        ...
        class AMToAM {
            ...
            @DisplayName("should invoke services in order for revoking access to user")
            void shouldInvokeServicesInOrder() {
                caseUserRepository.revokeAccess(AM_CASE_TYPE_ID, AM_CASE_ID, USER_ID, CREATOR.getRole());

                InOrder inOrder = inOrder(caseDetailsRepository, amSwitch, ccdCaseUserRepository, amCaseUserRepository);
                assertAll(
                    () -> inOrder.verify(amSwitch).isWriteAccessManagementWithCCD(AM_CASE_TYPE_ID),
                    () -> inOrder.verify(amSwitch).isWriteAccessManagementWithAM(AM_CASE_TYPE_ID),
                    () -> inOrder.verify(amCaseUserRepository).revokeAccess(AM_CASE_TYPE_ID, AM_CASE_ID, USER_ID, CREATOR.getRole()),
                    () -> inOrder.verifyNoMoreInteractions(),
                    () -> verifyNoMoreInteractions(amCaseUserRepository),
                    () -> verifyZeroInteractions(ccdCaseUserRepository)
                );
        ...
        class AMToBoth {
            ...
            @DisplayName("should invoke services in order for revoking access to user")
            void shouldInvokeServicesInOrder() {
                caseUserRepository.revokeAccess(BOTH_CASE_TYPE_ID, BOTH_CASE_ID, USER_ID, CREATOR.getRole());

                InOrder inOrder = inOrder(caseDetailsRepository, amSwitch, ccdCaseUserRepository, amCaseUserRepository);
                assertAll(
                    () -> inOrder.verify(amSwitch).isWriteAccessManagementWithCCD(BOTH_CASE_TYPE_ID),
                    () -> inOrder.verify(ccdCaseUserRepository).revokeAccess(BOTH_CASE_TYPE_ID, BOTH_CASE_ID, USER_ID, CREATOR.getRole()),
                    () -> inOrder.verify(amSwitch).isWriteAccessManagementWithAM(BOTH_CASE_TYPE_ID),
                    () -> inOrder.verify(amCaseUserRepository).revokeAccess(BOTH_CASE_TYPE_ID, BOTH_CASE_ID, USER_ID, CREATOR.getRole()),
                    inOrder::verifyNoMoreInteractions,
                    () -> verifyNoMoreInteractions(amCaseUserRepository),
                    () -> verifyNoMoreInteractions(ccdCaseUserRepository)
                );
