...
Feature: F-116: Retrieve audit events by case ID (V2)
  ...
  @S-116.4
  Scenario: must return caller supplied event summary and description in audit history
    Given a user with [an active profile in CCD]
    And a case that has just been created as in [Standard_Full_Case_Creation_Data]
    And a successful call [to create a token for case update event] as in [S-116.4-Prerequisite]
    And a successful call [to update the case with caller supplied event summary and description] as in [S-116.4-UpdateCase]
    When a request is prepared with appropriate values
    And the request [contains the Id of the case just created]
    And it is submitted to call the [Retrieve audit events by case ID] operation of [CCD Data Store]
    Then a positive response is received
    And the response [contains caller supplied event summary and description in audit history]
    And the response has all other details as expected
