...
Background:
  ...
  Scenario: must return negative response for Specific Case Event Data by External Parties
    ...
    And a successful call [to create a token for case creation] as in [Befta_Default_Token_Creation_Data_For_Case_Creation_External],
    And another successful call [to create a full case] as in [Befta_Case_Data_Extension_External],
    And another successful call [to get the details about case event for the case just created] as in [S-1024_Get_Case_Data_External],
...
Scenario: must return negative response for unauthorised access by Internal Parties
    ...
    Given a user with [an active profile in CCD],
      ...
      When a request is prepared with appropriate values,
      And the request [contains the reference of the case just created and the event id valid for that case],
      And it is submitted to call the [Retrieve a CaseView Event by case and event id for access to Internal Parties with unauthorised access ] operation of [CCD Data Store],
