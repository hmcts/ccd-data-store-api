...
Feature: F-105: Add Case-Assigned Users and Roles
  ...
  Scenario: must successfully increment Assigned User Count when assigning a user and case role for a specific case (by a user calling through/from an authorised application)
    ...
    And a successful call [to check the number of users having access to C1 in its supplementary data] as in [F-105-Prerequisite_Counter_Check_Call],
    When a request is prepared with appropriate values,
    ...
    Then a positive response is received,
    And the response has all the details as expected,
    And a call [to verify Olawale's reception of the role CR-1 and CR-2 over the case C1] will get the expected response as in [S-105.8842.1_Get_Case_Roles_for_Case_C1].
    #And a call [to verify the count of users assigned to C1 has increased by 1] will get the expected response as in [Verify Counter - 1],
    #And a call [to repeat the same request as above] will get the expected response as in [Repeated Call to Add Case-Assigned Users and Roles],
    #And a call [to verify the count of users assigned to C1 has NOT increased] will get the expected response as in [Verify Counter - 2],
    #And a call [to repeat the same request as above this time with a different user, Hemanth] will get the expected response as in [Repeated Call to Add Case-Assigned Users and Roles],
    #And a call [to verify the count of users assigned to a case has increased by 1] will get the expected response as in [Verify Counter - 3].
...
#     And a user [Olawale - with an active solicitor profile and valid User ID],
#     And a case [C1, which Richard has just] created as in [F-105_Case_Data_Create_C1],
#     And a successful call [to grant access for Olawale with a case role CR-1 over the case C1] as in [Prerequisite Access Grant Call],
#     #And a successful call [to check the number of users having access to C1 in its supplementary data] as in [F-105-Prerequisite_Counter_Check_Call],
#     When a request is prepared with appropriate values,
...
#     Then a positive response is received,
#     And the response has all the details as expected,
#     And a call [to verify Olawale's reception of the role CR-2 over the case C1] will get the expected response as in [Get Case Roles for Case C1].
#     #And a call [to verify the count of users assigned to a case] will get the expected response as in [the number of users having access to a case under the given organisation ID will not increase].
...
#     And a user [Dil - who is to add some case role assignment for a case],
#     And a user [Olawale - with an active solicitor profile and valid User ID],
#     And a successful call [by Richard to create a case - C1] as in [F-105_Case_Data_Create_C1],
#     #And a successful call [to check the number of users having access to C1 in its supplementary data] as in [F-105-Prerequisite_Counter_Check_Call],
#     When a request is prepared with appropriate values,
...
#     Then a positive response is received,
#     And the response has all the details as expected,
#     And a call [to verify Olawale's reception of the role CR-1 over the case C1] will get the expected response as in [Get Case Roles for Case C1],
#     #And a call [to verify the count of users assigned to a case] will get the expected response as in [the number of users having access to a case under the given organisation ID will not increase].