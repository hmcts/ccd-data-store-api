...
#    And a successful call [to create a token for case creation1] as in [S-131.5_CreateCase_Token_Creation]
#    Given a case that has just been created as in [F-131.5_CreateCase],
#    And a successful call [to create a token for event creation1] as in [S-131.5_Token_Creation]
#    And the request [attempts to create an event for the previously created case]
#    And it is submitted to call the [create event] operation of [CCD Data Store],
#    Then a negative response is received
#    And the response has all other details as expected.
