...
Feature: Create first scenarios for new Access Control to run on the pipeline as a spike
  ...
  @S-200.2
  Scenario: User's role assignments not defined for the current CaseType CT3J2 in J2 Jurisdiction
    Given a user with [a role assignment without definition for CaseType CT3J2 in J2 Jurisdiction]
    And a successful call [to create a token for case creation] as in [CT3J2_GetToken]
    When a request is prepared with appropriate values
    And the request [attempts to create a case for CT3J2 in J2 Jurisdiction]
    And it is submitted to call the [create case] operation of [CCD Data Store]
    Then a negative response is received
    And the response has all other details as expected
  ...
  Scenario: User's role assignments have readOnly settings on AccessProfile for the current CaseType's in the current Jurisdiction
    ...
    And a successful call [to create a token for case creation] as in [CT4_GetToken]
    When a request is prepared with appropriate values
  ...
  Scenario: User's role assignments have readOnly settings on AccessProfile for the current CaseType's in the current Jurisdiction
    ...
    And a successful call [to create a token for case creation] as in [CT5_GetToken]
    When a request is prepared with appropriate values
  ...
  Scenario: User's role assignments have readOnly settings on AccessProfile for the current CaseType's in the current Jurisdiction
    ...
    And a successful call [to create a token for case creation] as in [CT6_GetToken]
    When a request is prepared with appropriate values
