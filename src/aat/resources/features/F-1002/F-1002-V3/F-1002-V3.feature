...
Feature: F-1002-V3: Submit Event for an Existing Case (V3)
  ...
  Scenario: must successfully update case data content with Id and Token for a new document
    ...
    And another successful call [to get an update event token for the case just created]  as in [S-1011-V3_Update_Event_Token_Creation],
  ...
  Scenario: must successfully update case data content without any document field
    ...
    And another successful call [to create a token for case creation] as in [Befta_Jurisdiction2_Default_Token_Creation_Data_For_Case_Creation],
    And another successful call [by same user to create a case of this case type] as in [F-1002-V3-Case_Creation_Data],
    And another successful call [to get an update event token for the case just created] as in [S-1012-V3_Update_Event_Token_Creation],
    When a request is prepared with appropriate values,
    And the request [contains an Event Id received from upstream],
  ...
  Scenario: must successfully update case data content for multiple documents
    ...
    And another successful call [to get an update event token for the case just created]  as in [S-1013-V3_Update_Event_Token_Creation],
    When a request is prepared with appropriate values,
    And the request [contains an Event Id received from upstream],
  ...
  Scenario: must get an error response for a wrong hash token with out any change applied to Case
    ...
    And another successful call [by same user to create a case of this case type] as in [F-1002-V3-Case_Creation_Data],
    And another successful call [to upload a document with mandatory metadata] as in [Default_Document_Upload_Data],
    And another successful call [to get an update event token for the case just created]  as in [S-1014-V3_Update_Event_Token_Creation],
    When a request is prepared with appropriate values,
    And the request [contains an Event Id received from upstream],
  ...
  Scenario: must get an error response for a non existing document Id
    ...
    And another successful call [by same user to create a case of this case type] as in [F-1002-V3-Case_Creation_Data],
    And another successful call [to upload a document with mandatory metadata] as in [Default_Document_Upload_Data],
    And another successful call [to get an update event token for the case just created]  as in [S-1015-V3_Update_Event_Token_Creation],
    When a request is prepared with appropriate values,
  ...
  Scenario: must get an error response for a malformed document Id
    ...
    And another successful call [by same user to create a case of this case type] as in [F-1002-V3-Case_Creation_Data],
    And another successful call [to upload a document with mandatory metadata] as in [Default_Document_Upload_Data],
    And another successful call [to get an update event token for the case just created]  as in [S-1016-V3_Update_Event_Token_Creation],
    When a request is prepared with appropriate values,
  ...
  Scenario: must get an error response for a non existing Case Id
    ...
    And another successful call [by same user to create a case of this case type] as in [F-1002-V3-Case_Creation_Data],
    And another successful call [to upload a document with mandatory metadata] as in [Default_Document_Upload_Data],
    And another successful call [to get an update event token for the case just created]  as in [S-1017-V3_Update_Event_Token_Creation],
    When a request is prepared with appropriate values,
  ...
  Scenario: must get an error response for a malformed Case Id
    ...
    And another successful call [by same user to create a case of this case type] as in [F-1002-V3-Case_Creation_Data],
    And another successful call [to upload a document with mandatory metadata] as in [Default_Document_Upload_Data],
    And another successful call [to get an update event token for the case just created]  as in [S-1018-V3_Update_Event_Token_Creation],
    When a request is prepared with appropriate values,
