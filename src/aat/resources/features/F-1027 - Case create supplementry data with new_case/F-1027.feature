@F-1027
Feature: F-1027: Submit Case Creation Handle OrganisationProfileField with newCase
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  Scenario: Submit Event is invoked on v1_external#/case-details-endpoint/createCaseEventForCaseWorkerUsingPOST and case_data has organisationProfile with newCase set to YES
    Given a user with [an active profile in CCD]
    And a successful call [to create a case] as in [F-1027_CreateCasePreRequisiteCaseworker]
    And   a successful call [to get an event token for the case just created] as in [S-1027-GetUpdateEventToken]
    When a request is prepared with appropriate values
    And the request [contains a case Id that has just been created as in F-1027_CreateCasePreRequisiteCaseworker]
    And the request [contains an event token for the case just created above]
    And the request [contains some OrganisationPolicy fields with all correct values]
    And the request [is of caseType where case_data case data has Organisation.OrganisationID and organisationProfile with newCase set to Yes]
    And the request [specifying the case to be updated, as created in F-1027_CreateCasePreRequisiteCaseworker]
