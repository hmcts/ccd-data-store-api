...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  @S-034.4
  Scenario: must restore missing fields with R=false and C=true

    Given a user with [an active profile in CCD],

    When a request is prepared with appropriate values,
    And the request [omits a field with R=false and C=true],
    And it is submitted to call the [Validate case data] operation of [CCD Data Store],

    Then a positive response is received,
    And the response [has 200 return code],
    And the response [includes the previously omitted field],
    And the response has all other details as expected.

  #-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  @S-034.5
  Scenario: must restore partial missing fields with R=false and C=true
  ...
  #-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  @S-034.6
  Scenario: must NOT restore missing fields when conditions for restoration are not met

    Given a user with [an active profile in CCD],

    When a request is prepared with appropriate values,
    And the request [omits a field with R=false and C=true that does NOT match restoration criteria],
    And it is submitted to call the [Validate case data] operation of [CCD Data Store],

    Then a positive response is received,
    And the response [has 200 return code],
    And the response [does NOT include the previously omitted field],
    And the response has all other details as expected.
