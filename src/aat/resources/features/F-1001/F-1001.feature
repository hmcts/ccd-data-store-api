...
Feature: F-1001: Get case document metadata for a given caseId and documentId
  ...
  @S-1001
  Scenario: must receive document metadata and list of permissions for a valid Solicitor
  ...
  @S-1002
  Scenario: must receive an empty list of permissions in response when Solicitor user doesn't have valid access
    Given a user with [an active Solicitor profile in CCD with a specific variation of ACLs on a case type]
    And a user with [an active solicitor profile with another specific variation of ACLs on the same case type]
    And a user with [an active Solicitor profile having full permissions on the same case type]
    And a successful call [to create a token for case creation] as in [Befta_Jurisdiction2_Default_Token_Creation_Data_For_Case_Creation]
    ...
    When a request is prepared with appropriate values
    And the request [has the case id just created where a document id is associated]
    And the request [contains active Solicitor profile who does not have access]
    And it is submitted to call the [get document metadata] operation of [CCD Data Store]
    Then a positive response is received
    And the response [contains an HTTP 200 OK status code]
    And the response [contains the document metadata and empty list of permissions]
    And the response has all other details as expected

  @S-1010
  Scenario: must receive a document metadata and list of permissions for a valid Citizen user
    Given a user with [an active Citizen profile in CCD with access to a document]
  ...
  @S-1011
  Scenario: must receive an empty list of permissions when Citizen user doesn't have valid access
    Given a user with [an active Citizen profile in CCD with no access to a document]
    And a successful call [to create a token for case creation] as in [Befta_Jurisdiction2_Default_Token_Creation_Data_For_Citizen_Case_Creation]
    And another successful call [by a privileged user with full ACL to create a case of this case type] as in [Befta_Jurisdiction2_Default_Citizen_Case_Creation_Data]
    When a request is prepared with appropriate values
    And the request [has the case id just created where a document id is associated]
