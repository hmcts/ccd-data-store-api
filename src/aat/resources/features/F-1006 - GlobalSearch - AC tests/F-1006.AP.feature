...
Feature: F-1006: Global Search - Access Control Tests - AccessProcess
  ...
  @S-1006.AP.2 @AC2
  Scenario: Scenario 2.1 - CHALLENGED Access Process (region filter on STANDARD RoleToAccessProfile for case-type)
    Given a user with [a caseworker with RA1: BASIC role assignment and RA2: STANDARD role assignment with region attribute with value 123]
      ...
      And a successful call [to create a case as a citizen: for case type with AccessProfile region filter: region = 123] as in [F-1006_CreateCasePreRequisiteCitizen__FT_GlobalSearch_AC_2__Region_123]
      And a wait time of [5] seconds [to allow for Logstash to index the case just created].

    When a request is prepared with appropriate values,
     And the request [search for case type with STANDARD RoleToAccessProfile and case has unmatched region for user],
     And the request [search for case type with STANDARD RoleToAccessProfile and case has matching region for user],
     And it is submitted to call the [Global Search] operation of [CCD Data Store],

    Then a positive response is received,
     And the response [contains case with Access Process value CHALLENGED when unmatched region],
     And the response [contains case with Access Process value NONE when matching region],
     And the response has all other details as expected.
