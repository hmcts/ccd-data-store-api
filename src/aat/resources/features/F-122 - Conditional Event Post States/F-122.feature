...
#  CaseCreated -> CaseUpdated(TextField="updated" AND EmailField="*"):1;CaseAmended(TextField="amended" AND EmailField="*"):2;CaseRevoked(TextField="revoked" AND EmailField="*"):3;CaseDeleted
#
...
#-----------------------------------------------------------------------------------------------------------------------
  ...
  Scenario: *(FieldA) will keep the state as is (End state: CaseCretated)
    ...
    Then a positive response is received,
    And the response [contains state: CaseCretated, updated values for TextField, EmailField along with an HTTP-201 Created],
...
#-----------------------------------------------------------------------------------------------------------------------
  ...
  Scenario: Sets the state defined with a matching post state condition using != operator (End state: CaseRevoked2)
    ...
    Then a positive response is received,
    And the response [contains state: CaseRevoked2, updated values for TextField, EmailField along with an HTTP-201 Created],
