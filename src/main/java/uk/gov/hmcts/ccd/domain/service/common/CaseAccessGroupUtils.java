...
public class CaseAccessGroupUtils {
    ...
    public void updateCaseAccessGroupsInCaseDetails(CaseDetails caseDetails, CaseTypeDefinition caseTypeDefinition) {
        ...
        if (!caseDetails.getData().isEmpty()) {
            ...
            for (AccessTypeRolesDefinition acd : filteredAccessTypeRolesDefinitions) {
            ...
            //JsonNode node = mapper.convertValue(groups.caseAccessGroupsAsCollection(), ArrayNode.class);
    ...
    private void removeCCDAllCasesAccessFromCaseAccessGroups(CaseDetails caseDetails) {
        ...
        for (int i = 0; i < caseAccessGroupsJsonNodes.size(); i++) {
            final JsonNode valueNode = caseAccessGroupsJsonNodes.get(i).get("value");
        ...
        if (caseDetails.getData().get(CASE_ACCESS_GROUPS).isEmpty()) {
