...
/**
 * This builder builds retries context based on the spreadsheet config for retries timeouts. The intervals are fixed and are multiples of 3 starting from
 * 0s and 1 i.e. 0s, 1s, 3s, 9s, 27s ...
 *
...
public class CallbackRetryContextBuilder {
    ...
    private void buildCustomRetryContext(final List<Integer> callbackRetryTimeouts, final List<CallbackRetryContext> retryContextList) {
        ...
        if (!callbackRetryTimeouts.isEmpty()) {
            ...
            if (!callbackRetryTimeouts.isEmpty()) {
                retryContextList.add(new CallbackRetryContext(defaultCallbackRetryIntervalsInSeconds.get(2), callbackRetryTimeouts.remove(0)));
                callbackRetryTimeouts.forEach(callbackRetryTimeout ->
