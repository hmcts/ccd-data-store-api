...
public abstract class CaseViewFieldProcessor extends FieldProcessor {
    ...
    public CaseViewField execute(CaseViewField caseViewField) {
        ...
        if (BaseType.get(COMPLEX) == fieldType) {
            return executeComplex(caseViewField, caseViewField.getId());
    ...
    protected CaseViewField executeComplex(CaseViewField caseViewField, String fieldPrefix) {
        caseViewField.setDisplayValue(
            caseViewField.getValue() instanceof ObjectNode ?
                executeComplex((ObjectNode) caseViewField.getValue(), caseViewField.getFieldType().getComplexFields(), null, null, fieldPrefix) :
