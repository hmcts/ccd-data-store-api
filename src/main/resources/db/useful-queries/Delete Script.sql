...
caseTypeIds constant text[] := array['AAT','AAT_AUTH_15','AAT_AUTH_2','AAT_AUTH_4','AAT_AUTH_5','AAT_AUTH_8','AAT_PRIVATE','AAT_PRIVATE_B''AllDataTypes2','CaseProgression',
...
CREATE TEMP TABLE tmp_case_data_ids ON COMMIT DROP AS
		   SELECT id
			...
			WHERE cd.last_modified <= 'now'::timestamp - '3 MONTH'::interval
			  AND case_type_id = any(caseTypeIds);
