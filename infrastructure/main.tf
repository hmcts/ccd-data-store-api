locals {
  ...
  elastic_search_host = "${var.elastic_search_enabled == "false" ? "" : "${format("http://%s:9200", join("", data.azurerm_key_vault_secret.ccd_elastic_search_url.*.value))}"}"
...
module "ccd-data-store-api" {
  ...
  app_settings = {
    ...
    ELASTIC_SEARCH_HOSTS                = "${local.elastic_search_host}"
